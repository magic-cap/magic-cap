# Diving Deeper

The [Overview](./overview.md) glosses over or doesn't discuss several other details.

[Locating and Retriving Data](./locations.md)
  : Locating the correct Data to go with a Read Cap will be required by many applications

[Retrieving Data via Network](./network-api.md)
  : An API to retrieve Data for a particular Read Cap from an HTTP server

[Catalogs and Anthologies](catalog-anthology.md)
  : Two different ways to consider groups of encrypted Data.

