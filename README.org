...
The resulting (fixed, tiny) Magic Cap string can be redeemed later for the secret data.
It also has other interesting, cryptographically-ensured features (described later).

The "Magic Cap" string is short (73 bytes) and can fit in TPMs or other secure storage.
...
#+RESULTS: encrypt
: mcap:0:r:rvhme815HvTPwrkDpKk8tudmXcDTSxg3-iOa20PWoVI:PO0xAgEbu9c8Pi4oxXdqzg

Now, we have two things: "a Data File" and "a Magic Cap string".
The latter is =mcap:0:r:<base64-encoded hash>:<base64-encoded key>=
...
#+RESULTS:
: Wrote 9621 bytes of plaintext to "plain".
...
You can encrypt the proofs with magic-cap and upload the encrypted file somewhere, and then send a single line of 73 characters to your client so they can decrypt the proofs on their computer.
...
The mcap to unlock it is: =mcap:0:r:UHEpVWu1xzvwZyQiMul4EtGTTD0mPOPSaeYDMVRMSj4:bd9d-FmYZtbFuSUYGRo2Og=
If you're using "cargo" a command-line to do that is:

#+begin_src sh
  cargo run -- decrypt --ciphertext kitten.mcap --cap mcap:0:r:UHEpVWu1xzvwZyQiMul4EtGTTD0mPOPSaeYDMVRMSj4:bd9d-FmYZtbFuSUYGRo2Og --plaintext kitten.jpeg
#+end_src
...
#+begin_src sh
  cargo run -- verify --ciphertext kitten.mcap --cap mcap:0:v:UHEpVWu1xzvwZyQiMul4EtGTTD0mPOPSaeYDMVRMSj4
#+end_src
...
- The Magic Cap string, 75 characters starting with =mcap:0:r:...=
